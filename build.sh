...
echo ""
echo "building. . ."
dotnet build "UltraRogue.sln"

userpath=$(< "./resources/buildsettings" )
if [ -d "${userpath}plugins/UltraRogue/" ]; then
    echo "destroying existing files"
    rm -r -f "${userpath}plugins/UltraRogue/"
fi

echo "copying build to ultrakill folders"
mkdir "${userpath}plugins/UltraRogue/"
cp "./bin/Debug/netstandard2.1/redrain0o0.UltraRogue.dll" "${userpath}plugins/UltraRogue/"
echo "done! ! !"
